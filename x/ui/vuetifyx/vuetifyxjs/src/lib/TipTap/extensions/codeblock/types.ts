  ...
  addKeyboardShortcuts() {
    return {
      ...
      Enter: ({ editor }) => {
        ...
        const firstLinePos = text.indexOf('\n')
        ...
        if (firstLinePos < 0) {
          ...
          if (m && m[2]) {
            m[2].replace(/(^\s+|\s+$)/g, '').split(',').map(value => {
              const kv = value.replace(/(^\s+|\s+$)/g, '').split('=')
              if (kv.length == 2) {
                const k = kv[0]
                let v = kv[1]
                if (k == "lang") {
          ...
          const changedOptions = Object.keys(attrs).filter(k => {
            return attrs[k] != $from.parent.attrs[k]
          })
          if (changedOptions.length) {
            editor.commands.updateAttributes(this.type, attrs);
          }
        }
